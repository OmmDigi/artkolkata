...
import ProductCard from "@/app/Component/trending/ProductCard";

// The stored product shape varies (a guest save keeps whatever the card was
// handed, the API read returns the product row), so every field falls back
// through each key it is known to arrive as before reaching the card.
const itemSlug = (item: any) =>
  item.slug || item.href?.split("/product/")[1] || item.url?.split("/product/")[1];

const itemImage = (item: any) => item.images?.[0]?.image || item.image1 || item.image;

const itemHoverImage = (item: any) =>
  item.images?.[1]?.image || item.image2 || itemImage(item);

const itemPrice = (item: any) => item.selling_price ?? item.price ?? 0;

// The card reads variants off the product it is given and passes that same
// object to the cart, so the two known variant keys are merged into one shape.
const normalizeItem = (item: any) => ({
  ...
  images: item.images?.length
    ? item.images
    : [{ image: itemImage(item) }, { image: itemHoverImage(item) }].filter(
        (i) => i.image,
      ),
    ...
    <div className="flex flex-col items-center justify-center py-12 sm:py-16 px-4">
      <div className="mb-6">
        <svg
          ...
          viewBox="0 0 146 160"
          className="w-24 sm:w-32 md:w-36 h-auto text-gray-900"
        >
          <path
      ...
      <Link href={"/product"} className="w-full sm:w-auto">
        <button className="w-full sm:w-auto px-8 py-3 bg-[#02F8C5] text-black text-sm font-medium rounded transition-colors">
          Start Shopping
        </button>
      </Link>
    </div>
  ...
  return (
    <>
      <section className="bg-white border-b border-gray-100">
        <div className="max-w-7xl text-center mx-auto px-4 py-8 sm:py-10 md:py-12">
      ...
      <div className="min-h-screen bg-white px-1 md:px-6">
        <div className="max-w-7xl mx-auto px-4 py-6 sm:py-8">
          <p className="text-sm text-gray-600 mb-6">
            Showing {wishlist?.length ?? 0} saved{" "}
            {wishlist?.length === 1 ? "product" : "products"}
          </p>

          {/* Same grid as the product listing page, filled with the same card. */}
          <div className="grid gap-4 md:gap-3 grid-cols-2 md:grid-cols-2 lg:grid-cols-4">
            {wishlist.map((item) => {
              const product = normalizeItem(item);

              return (
                <div key={product.id} className="shadow-2xl rounded-lg">
                  <ProductCard
                    product={product}
                    ids={product.id}
                    image={itemImage(product)}
                    hoverImage={itemHoverImage(product)}
